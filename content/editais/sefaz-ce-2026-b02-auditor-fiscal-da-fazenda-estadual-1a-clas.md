---
id: sefaz-ce-2026-b02-auditor-fiscal-da-fazenda-estadual-1a-clas
status: parcial
peso: 5
data_processamento: 11/07/2026
edital_arquivo: SEFAZ-CE.pdf
orgao: ''
sigla: SEFAZ-CE
esfera: ''
estado: ''
cidade: ''
banca: ''
edital_numero: ''
edital_ano: 2026
edital_link: ''
cargo: B02- Auditor-Fiscal da Fazenda Estadual 1ª Classe – Área de Conhecimento –
  Tecnologia da Informação
area: ''
escolaridade: Nível Superior
formacao_exigida: ''
vagas_total: null
regime: ''
jornada_semanal: ''
local_trabalho: ''
remuneracao:
  gratificacoes: []
beneficios: {}
carreira: {}
titulacao: {}
trabalho: {}
localizacao: {}
notas: {}
fontes:
- tipo: Oficial
  descricao: Edital SEFAZ-CE.pdf
  link: ''
  data: 11/07/2026
dados_faltantes:
- campo: orgao
  descricao: Nome completo do órgão
- campo: banca
  descricao: Banca organizadora
- campo: regime
  descricao: Regime jurídico
- campo: jornada_semanal
  descricao: Jornada semanal
- campo: remuneracao.vencimento_base
  descricao: Vencimento base inicial (R$)
- campo: remuneracao.remuneracao_total_inicial
  descricao: Remuneração total inicial (R$)
- campo: beneficios.aux_alimentacao
  descricao: Auxílio alimentação (R$/mês)
observacoes_usuario: Banca Fcc
---

## Análise Automática – SEFAZ-CE 2026 – B02- Auditor-Fiscal da Fazenda Estadual 1ª Classe – Área de Conhecimento – Tecnologia da Informação

**Processado automaticamente em 11/07/2026**

### Dados Extraídos

| Campo | Valor |
|:------|------:|
| Órgão | ⚠️ Não encontrado |
| Cargo | B02- Auditor-Fiscal da Fazenda Estadual 1ª Classe – Área de Conhecimento – Tecnologia da Informação |
| Banca | ⚠️ Não encontrada |
| Regime | ⚠️ Não encontrado |
| Venc. Base | ⚠️ Não encontrado |
| Remun. Total | ⚠️ Não encontrado |
| Aux. Alimentação | ⚠️ Não encontrado |

### ⚠️ Dados Pendentes

Os seguintes campos não foram encontrados automaticamente:

- [ ] Nome completo do órgão (`orgao`)
- [ ] Banca organizadora (`banca`)
- [ ] Regime jurídico (`regime`)
- [ ] Jornada semanal (`jornada_semanal`)
- [ ] Vencimento base inicial (R$) (`remuneracao.vencimento_base`)
- [ ] Remuneração total inicial (R$) (`remuneracao.remuneracao_total_inicial`)
- [ ] Auxílio alimentação (R$/mês) (`beneficios.aux_alimentacao`)

### Observações do Usuário

Banca Fcc
